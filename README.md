...
### 测试：JAVXX 搜索图片抓取（基于关键字文件）

目标：
- 站点 `https://javxx.com/cn/search?keyword=...` 为搜索结果页；根据根目录的 `keywords.txt`（每行一个关键字），对每个关键字访问对应搜索页，从主内容区 `class="vid-items"` 下的第一个 `class="item"` 的 `class="img" -> class="poster" -> class="image"` 节点里提取 `<img src>`，下载图片并以关键字作为文件名保存。

准备：
- `keywords.txt`：位于项目根目录，每行一个关键字，示例：
```
ipx-770
ipx-123
```
- 新增插件：已提供 `plugins/javxx_search_image_plugin.py`

配置示例（建议另存为 `config\javaxx.json`）：
```json
{
  "allowed_domains": ["javxx.com"],
  "max_concurrency": 2,
  "per_domain_delay_ms": 800,
  "respect_robots_txt": true,
  "plugins": ["plugins.javxx_search_image_plugin"],
  "storage": {"output_dir": "output", "sqlite_path": "crawler.db"},
  "seeds": [],
  "seeds_from_keywords": {
    "file": "keywords.txt",
    "template": "https://javxx.com/cn/search?keyword={kw}"
  }
}
```

自动生成 seeds：
- 新增字段 `seeds_from_keywords`，运行时自动从 `keywords.txt` 读取关键字并按模板拼接为 URL，追加到 `seeds`。

运行（Windows 路径示例）：
```bash
python -m crawler.cli --config config\javaxx.json
```

流程说明（一步一步）：
- 读取 `config.javxx.json`：加载并发、速率、允许域名、插件列表等。
- 以每个关键字构造的搜索URL为种子（seeds）开始抓取。
- 命中插件 `plugins.javxx_search_image_plugin`：
  - 从 URL 的查询参数中解析 `keyword`（用于图片文件名）。
  - 使用 BeautifulSoup 定位 `.vid-items .item .img .poster .image img`，取 `src`。
  - 若初次未解析到，尝试 JS 渲染回退（环境安装 Playwright 才会生效）。
  - 计算图片绝对URL，使用抓取器下载图片二进制。
  - 根据 URL 或响应头推断扩展名（.jpg/.png/...），保存到 `output/images/<keyword>.<ext>`。
  - 同时保存 JSON 元数据（图片URL、保存路径、关键字）到 `output` 目录。
- 结束：不继续扩展链接（此插件返回空链接列表）。

输出结果：
- 图片：`output/images/<keyword>.<ext>`
- JSON：`output/<搜索URL安全化>.json`（包含 keyword、image_url、saved_path 等）

使用建议与注意事项：
- 关键词较多时，建议先小量测试，再扩大 `seeds` 列表。
- 若页面为JS渲染而未解析到图片，安装 Playwright 并重试；或延长 `per_domain_delay_ms`。
- 遵守站点政策与 robots.txt；降低并发与速率避免触发风控。

去重与覆盖：
- 默认开启去重：若已存在以关键字命名的图片，则跳过再次下载。
- 可通过 `plugin_params.javxx_search_image_plugin` 配置覆盖行为。
示例：
```json
{
  "plugins": ["plugins.javxx_search_image_plugin"],
  "plugin_params": {
    "javxx_search_image_plugin": {
      "skip_if_exists": true,
      "overwrite": false
    }
  }
...
```
- `skip_if_exists`: 已存在图片时跳过下载（默认 `true`）。
- `overwrite`: 总是重新下载并覆盖已有图片（默认 `false`）。

仅抓取首页：
- 若只需处理每个关键字的首个搜索结果页，可在站点配置中启用 `disable_global_link_extraction`，将抓取限制为种子页与插件返回的链接，不再从页面中发现和跟随其他链接（包括分页）。
示例（`config\\javaxx.json`）：
```json
{
  "allowed_domains": ["javxx.com"],
  "plugins": ["plugins.javxx_search_image_plugin"],
...
```
- 即使未启用上述开关，基础链接发现也会过滤含 `?page=` 的分页链接，以避免误跟随分页。
